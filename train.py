...
from models import get_model, extract_features, freeze_finetune, freeze_fc
import random
import numpy as np
...
def train(model, dataloader, bd_dataloader, criterion, optimizer, use_cuda, freeze, use_centerloss):
    model.train()
    centerloss = CenterLoss(200, 512)
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        # print(outputs.shape)
        if use_centerloss:
            features, outputs = extract_features(model, inputs)

            ct_loss = 0.001 * centerloss(features, targets)

            loss = criterion(outputs, targets) + ct_loss
        else:
            outputs = model(inputs)
        ...
        if use_centerloss:
            ctloss.update(ct_loss.item(), inputs.size(0))
        ...
        bar.suffix = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | ' \
                     'Loss: {loss:.4f} | CenterLoss: {ctloss:.4f}| top1: {top1: .4f} | top5: {top5: .4f}'.format(
            batch=batch_idx + 1,
            size=len(dataloader),
            data=data_time.avg,
            bt=batch_time.avg,
...
def main(args):
    ...
    freeze = args.freeze
    use_centerloss = args.use_centerloss
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc = train(model, train_loader, bd_train_loader, criterion, optimizer, use_cuda, freeze,
                                      use_centerloss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_centerloss', default=False, type=bool)
    # Checkpoints
    parser.add_argument('-c', '--checkpoint', default='checkpoint', type=str, metavar='PATH',
                        help='path to save checkpoint (default: checkpoint)')
    parser.add_argument('--resume', default='', type=str, metavar='PATH',
                        help='path to latest checkpoint (default: none)')
    parser.add_argument('--freeze', default=False, type=bool, help='freeze backbone for phase2'
                        )
    # Miscs
